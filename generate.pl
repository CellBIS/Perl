...
my $command    = CellBIS::DockerPerl::Command->new(images_name => $image_name, dir_location => getcwd);
...
sub list_docker_images {
  ...
  for my $dl (@data) {
    ...
    push @result, {name => $for_list_docker[0], tag => $for_list_docker[1], hash => $for_list_docker[2]};
  }
